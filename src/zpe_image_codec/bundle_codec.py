...
class BundleRoute:
    ...
    def to_dict(self) -> dict[str, object]:
        ...
        payload["bundle_word_count"] = (
            payload["base_word_count"] + payload["fiber_word_count"] + payload["state_word_count"]
        )
        return payload
...
def route_bundle_image(image: np.ndarray) -> BundleRoute:
    ...
    if not sparse_decision.accept:
        reasons.append("sparse_route_not_admissible")
    if hole_count < 1:
...
def bundle_metrics(route: BundleRoute, image: np.ndarray) -> dict[str, object]:
    if not route.accepted:
        return {
            ...
            "topological_bytes_20bit": packed_word_bytes(route.base_words),
            "sparse_bytes_20bit": packed_word_bytes(route.fiber_words),
            "quadtree_bytes_20bit": packed_word_bytes(encode_enhanced(np.asarray(image, dtype=np.uint8), bit_depth=3)[0]),
    ...
    enhanced_words, _ = encode_enhanced(np.asarray(image, dtype=np.uint8), bit_depth=3)
    return {
        "accepted": True,
        "bundle_bytes_20bit": packed_word_bytes(route.bundle_words),
        "topological_bytes_20bit": packed_word_bytes(route.base_words),
        "sparse_bytes_20bit": packed_word_bytes(route.fiber_words),
        "quadtree_bytes_20bit": packed_word_bytes(enhanced_words),
